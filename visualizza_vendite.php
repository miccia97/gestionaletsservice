...
<style>
  ...
  :root {
      --accent-color: #2563eb; /* blu acceso */
      --bg-color: #f9fafb; /* sfondo chiaro */
      --text-color: #111827; /* quasi nero */
      --muted-text: #6b7280; /* grigio */
      --border-color: #e5e7eb; /* grigio chiaro */
      --radius: 12px;
...
.table-row {
  ...
  /* Rimuovo freccia svg dal bottone dettagli */
  .btn-actions svg {
      display: none;
  }
...
<div><?= htmlspecialchars($row['id_cliente']) ?> - <?= htmlspecialchars(trim($row['cliente_nome'] . ' ' . $row['cliente_cognome'])) ?: '<em>Non definito</em>' ?></div>
